...
document.body.onresize = () => {
    Object.keys(i_m).forEach((idx)=>{
        ...
        if((idx%2) == 1)
            i_m[idx].draw_prop.reset = false;
    });
};

Object.keys(i_m).forEach(async (idx)=>{
    idx = +idx;
    ...
    if((idx%2) == 0){
        ...
        i_m[idx].draw_prop = {
            ...
            set reset(draw){
                ...
                draw ? undefined : drawCanvas([i_m[idx],i_m[idx+1]]);
            },
            set pan(panFac){
                ...
                panFac.draw ? undefined : drawCanvas([i_m[idx],i_m[idx+1]]);
            },
            set scale(scale_p){
                let change = +((Math.max(Math.min(this.scale_curr + (this.scaleFac * scale_p.scroll),10.0),0.05)).toFixed(this.multiplier));
                ...
                scale_p.draw ? undefined : drawCanvas([i_m[idx],i_m[idx+1]]);
            }
        };
    }
    else{
        ...
        await readFile(i_m[idx-1]);
        await readFile(i_m[idx]);
    }
});
...
function c_wheel(e){
    ...
    let canvas = e.target;
    if(e.ctrlKey){
        canvas.draw_prop.scale = {x: (e.offsetX), y: (e.offsetY), scroll:(e.deltaY<0? 1 : -1)};
    }
}
...
async function readNIFTI(canvas, data) {
    // parse nifti
...
async function readFile(canvas) {
    ...
    var blob = makeSlice(data, 0, data.size);

    var reader = new FileReader();

    reader.onloadend = function (evt) {
        if (evt.target.readyState === FileReader.DONE) {
            readNIFTI(canvas, evt.target.result);
        }
    };
