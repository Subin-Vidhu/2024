...
file_path = r'c:\Users\Subin-PC\Downloads\Access history report - 01-07-2024 to 29-07-2024.xlsx'
...
total_lack_time = 0
total_lack_office_hours_time = 0
...
total_lack_time_excluding_weekends = 0
total_lack_office_hours_time_excluding_weekends = 0
...
for name, group in df.groupby('Name'):
    ...
    for date, group_by_date in group.groupby('Date'):
        ...
        lack_time = standard_workday_seconds - daily_total_time
        lack_office_hours_time = standard_workday_seconds - daily_office_hours_time
        ...
        if lack_time > 0:
            total_lack_time += lack_time
        if lack_office_hours_time > 0:
            total_lack_office_hours_time += lack_office_hours_time
        ...
        if not is_weekend(date):
            ...
            if lack_time > 0:
                total_lack_time_excluding_weekends += lack_time
            if lack_office_hours_time > 0:
                total_lack_office_hours_time_excluding_weekends += lack_office_hours_time
...
def print_lack_time_spent(time_spent, title):
    print(title)
    for name, dates in time_spent.items():
        print(f"\n{name}:")
        for date in sorted(dates):
            total_seconds = dates[date]['total_time']
            lack_time = standard_workday_seconds - total_seconds
            if lack_time > 0:
                hours, remainder = divmod(lack_time, 3600)
                minutes, seconds = divmod(remainder, 60)
                print(f"  {date.strftime('%b %d, %Y')}: {int(hours)} hours, {int(minutes)} minutes, {int(seconds)} seconds lack")
            else:
                print(f"  {date.strftime('%b %d, %Y')}: No lack of time")
...
def print_total_lack_time(title, total_lack_time):
    print(title)
    hours, remainder = divmod(total_lack_time, 3600)
    minutes, seconds = divmod(remainder, 60)
    print(f"  {int(hours)} hours, {int(minutes)} minutes, {int(seconds)} seconds lack")
...
print_total_lack_time("\nTotal lack of time spent in office:", total_lack_time)
print_total_lack_time("\nTotal lack of time spent in office during office hours (8:30 AM to 6:45 PM):", total_lack_office_hours_time)
...
print_total_lack_time("\nTotal lack of time spent in office excluding weekends:", total_lack_time_excluding_weekends)
print_total_lack_time("\nTotal lack of time spent in office during office hours (8:30 AM to 6:45 PM) excluding weekends:", total_lack_office_hours_time_excluding_weekends)
# ```

# This code will now calculate and print the lack of time for each day and the total lack of time for the whole month, both including and excluding weekends.